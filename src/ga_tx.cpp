...
namespace sdk {
    namespace {
        ...
        static bool output_contributes_to_last_vbf(const nlohmann::json& output)
        {
            ...
            // but it isn't be generated if the output was already blinded, in which case the blinding_key is not set.
        ...
        for (const auto& output : transaction_outputs) {
            ...
            if (is_partial || (i < num_outputs - 1)) {
                ...
                for (const auto& scalar : scalars) {
                    const std::vector<unsigned char> v = ec_scalar_add(vbf, h2b(scalar));
                    std::copy(std::begin(v), std::end(v), vbf.begin());
                }
                output_vbfs.emplace_back(vbf);
            }
